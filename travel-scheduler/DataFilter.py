...
def append_food(hotel_lat, hotel_lon, attractions, df):
    ...
    hours_before_food = 0
    fin_itinerary = []
    restaurant = df.query_potential_locations(hotel_lat, hotel_lon, ["FOOD_BEVERAGE"], ["FOOD_BEVERAGE"], 15, 50)
    ...
    for attraction in attractions:
        ...
        if (hours_before_food > 6):
            ...
            center = lat + "," + lon
            restaurant = df.query_potential_locations(lat, lon, ["FOOD_BEVERAGE"], ["FOOD_BEVERAGE"], 15, 50)
            random.shuffle(restaurant)
            fin_itinerary.append(restaurant[0])
            fin_itinerary.append(attraction)
            hours_before_food = 0
        elif (hours_before_food > 4):
            restaurant = df.query_potential_locations(str(attraction.lat), str(attraction.lon), ["FOOD_BEVERAGE"], ["FOOD_BEVERAGE"], 15, 50)
            random.shuffle(restaurant)
            fin_itinerary.append(attraction)
            fin_itinerary.append(restaurant[0])
            hours_before_food = 0
...
def user_input_to_output(center, user_locs, interests, categories, days):
    dataFilter = DataFilter()
    lat, lon = center.split(",")[0], center.split(",")[1]
    attractions = dataFilter.query_potential_locations(lat, lon, interests, categories)
    ...
    printNicelyxD(fin_itinerary)
    print('Total time traveling: ' + str(SATime))
...
days = 3
user_input_to_output(center, user_locs, interests, categories, days)
